...
const mjMeta = require("./extract/image/mjmeta");
...
const extractData = async (options) => {
  ...
  const apiServerImagePreviewSizes = [1280, 800];
  ...
  return new Promise((resolve, reject) => {
    pipeline(
      ...
      processIndicator({
        onTick: ({ diff, lastTime }) =>
          log.info(
            lastTime,
            `Processed ${stats.processed} entries (#${options.skip + stats.processed}, +${diff}, processing ${
              stats.processing - stats.processed
            } and queued ${stats.queued - stats.processing} entries)`
          ),
      }),
